...
### 4.1 実装と Dart / TypeScript の同期（#88）

| 実装 | 対象 | 備考 |
|---|---|---|
| `functions/src/lib/name_key.ts` | `normalizeName`（手順 1〜3）+ `nameKeyOf`（ハッシュ化） | サマリーのドキュメント ID を導出する正 |
| `lib/core/utils/name_key.dart` | `normalizeName`（手順 1〜3）のみ | よく買う物テンプレートの重複判定（#43）に使用 |

Dart 側は **NFKC を含め TS 側と同一のアルゴリズム**を実装する（#88 で同期。NFKC は Dart の
標準ライブラリに無いため `unorm_dart` パッケージを使う）。両実装の出力一致は共通フィクスチャ
`test/fixtures/name_normalization_cases.json` を双方のテストが読むことで担保しており、
片側だけを変更すると必ずもう一方のテストが落ちる。

`nameKeyOf`（ハッシュ化）は Dart へ移植していない。`nameKey` は
`purchaseHistorySummaries` のドキュメント ID 専用であり、クライアントはこのコレクションを
読まないため（購入履歴画面 #42 が読むのは `itemHistory`）。クライアントからサマリーを
直接引く必要が生じた時点で移植する。
