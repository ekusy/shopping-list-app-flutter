...
> **実装メモ（2026-06, Issue #40 実装時点）**
>
> - モデルは本書 §4 の `gemini-2.5-flash` 想定から **Gemini 3.x Flash-Lite 系**に
>   更新（2.0 系は 2026-06-01 廃止済、2.5 系は 2026-10-16 廃止予定のため）。
>   `functions/src/data/gemini_client.ts` の `MODEL_ID` 単一定数で管理し、
>   デプロイ前に正確な公開モデル ID・料金を再確認するコメントを付与している。
> - SDK は `@google/genai`（Vertex backend）を採用（旧 `@google-cloud/vertexai` は
>   2026-06-24 サポート終了のため不使用）。
> - ファイル構成は #52 の3層分離に従い、
>   `lib/suggestions.ts`（純粋ロジック・vitest網羅）/
>   `data/suggestions_store.ts`（Firestore I/O・キルスイッチ含む）/
>   `data/gemini_client.ts`（Vertex呼び出し・1回リトライ）/
>   `triggers/suggestions.ts`（`onSchedule` 薄wrapper・`processGroup` 切り出し）
>   に実装した。
> - 未対応事項: `suggestions` コレクショングループへの Firestore TTL（90日）の
>   手動設定、SA への `roles/aiplatform.user` 付与・Vertex AI API 有効化、
>   Gemini 呼び出しのモック統合テスト、Phase 2（表示UI・FCM通知）。
