...
## (f) AI 週次提案フロー（#37 / #40 / #41）

クライアント無変更で履歴を裏側に蓄積し、週次バッチで AI 提案を生成、アプリ内で購読表示する。
バックエンドは Cloud Functions（`functions/`）、生成は Vertex AI Gemini。設計の詳細は
[`docs/ドラフト/AI提案機能/`](../ドラフト/AI提案機能/00-概要とロードマップ.md) と
[functions/README.md](../../functions/README.md) を参照。

```mermaid
flowchart LR
    subgraph Client["Flutter アプリ"]
        ItemUI["ItemCard / dashboard\n（購入・削除は既存のまま）"]
        SugBtn["AI 提案ボタン\n未読バッジ\n(hasUnreadSuggestionProvider)"]
        SugScr["SuggestionsScreen\n/suggestions (deferred)"]
    end
    subgraph FN["Cloud Functions v2"]
        TRG["onItemUpdated / onItemDeleted"]
        WK["weeklySuggestions\nonSchedule 土 08:00 JST"]
    end
    DBItems[("groups/{gid}/items")]
    HIST[("itemHistory\nTTL / purchaseHistorySummaries")]
    SUG[("groups/{gid}/suggestions/{weekId}")]
    GEMINI["Vertex AI Gemini"]

    ItemUI -->|write| DBItems
    DBItems -->|Firestore trigger| TRG
    TRG -->|アーカイブ＋集約| HIST
    WK -->|読み取り| DBItems
    WK -->|読み取り| HIST
    WK -->|プロンプト| GEMINI
    GEMINI -->|構造化 JSON| WK
    WK -->|保存| SUG
    SUG -->|"watchLatestSuggestion\nsnapshots"| SugBtn
    SUG -->|snapshots| SugScr
    SugScr -.->|markSuggestionAsRead| SugBtn
```

**ポイント**:
- **書き込み経路は既存のまま**: ユーザーの購入・削除は `groups/{gid}/items` への通常書き込み。
  Functions の Firestore トリガー（`onItemUpdated` / `onItemDeleted`）が物理削除でも削除時点の
  スナップショットを受け取り、`itemHistory`（生イベント・TTL）と `purchaseHistorySummaries`
  （商品別サマリー）をインクリメンタルに更新する（#37）。クライアント側のクエリ・削除フロー・
  テストには一切手を入れない。
- **週次生成**: `weeklySuggestions`（`onSchedule`、毎週土 08:00 JST）が履歴を集計して Gemini に渡し、
  構造化出力を `groups/{gid}/suggestions/{weekId}` に保存する（#40）。`system/config.suggestionsEnabled`
  をキルスイッチとして確認する。
- **クライアント購読**: `latestSuggestionProvider`（`StreamProvider<Suggestion?>`、
  `suggestion_providers.dart`）が `SuggestionRepository.watchLatestSuggestion` で最新提案を購読。
  `hasUnreadSuggestionProvider` が最新 `weekId` と `SharedPreferences` の既読 `weekId` を比較して
  未読バッジを制御し、`SuggestionsScreen` 表示時に `markSuggestionAsRead` で既読化して
  バッジ provider を invalidate する（#41）。提案はグループ共有のため、未読状態のみ端末ローカル
  （SharedPreferences）で管理する。

---

## (g) よく買う物フロー（#43）

**テンプレート購読**: `favoritesProvider`（`StreamProvider<List<FavoriteItem>>`、`favorite_providers.dart`）が
`FavoriteItemRepository.watchFavorites(gid)` で `groups/{gid}/favoriteItems` を order 昇順でリアルタイム購読する。

**重複判定と先頭挿入**: 一括追加時は `filterNewFavorites`（top-level 純粋関数）が `normalizeName` で
未購入アクティブ品名と突合し、重複するものをスキップ。非重複のものは現在の最小 order より小さい order を
選択順で割り当て、`ItemRepository.addItem` で買い物リストの先頭に挿入する。

`normalizeName`（`core/utils/name_key.dart`）は **NFKC 正規化 → トリム → 連続空白の圧縮 → 小文字化** の順に
処理する（#88）。全角英数・半角カナ・全角スペースが畳み込まれるため、`ＡＢＣ牛乳` と `ABC牛乳` は同一品名として
重複判定される。Functions 側の `functions/src/lib/name_key.ts` と**同一アルゴリズム**であり、両者の出力一致は
共通フィクスチャ `test/fixtures/name_normalization_cases.json` を双方のテストが読むことで担保している。

**画像（直接追加）**: よく買う物画面のフォームからの直接追加は、アイテム追加（(a)/§3.4）と同じ 2 段保存。
`addFavorite`（imageUrl 空）→ ドキュメント ID 確定 → `StorageRepository.uploadFavoriteImage`
（`groups/{gid}/favoriteItems/{favoriteId}.jpg`）→ `updateFavorite` で imageUrl 更新（best-effort）。
編集での画像差し替え／外しは `uploadFavoriteImage` / `deleteFavoriteImage`、削除時は孤児画像防止のため
`deleteFavoriteImage` を best-effort で実行する（favorites には Functions トリガが無いため）。一方、
**アイテムからの昇格**（UC-F2）は既存の `imageUrl`（Storage URL）をコピーするだけで新規ファイルは作成しない。
