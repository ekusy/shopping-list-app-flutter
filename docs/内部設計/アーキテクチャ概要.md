...
lib/
  domain/repositories/        ← 抽象 interface（Firebase 非依存）
    ...
    suggestion_repository.dart     SuggestionRepository（#41 AI 週次提案の読み取り）
    favorite_item_repository.dart  FavoriteItemRepository（#43 よく買う物テンプレート）
    history_repository.dart        HistoryRepository（#42 購入履歴の読み取り）
  data/repositories/          ← Firestore / Auth / Storage 実装
    ...
    firestore_suggestion_repository.dart
    firestore_favorite_item_repository.dart
...
sequenceDiagram
    ...
    participant Bar as DashboardAddBar
    participant Ctrl as ItemController
    participant Repo as ItemRepository
    participant DB as Firestore

    User->>Form: 入力して追加ボタン押下
    Form->>Bar: onAdd(draft, imageBytes)
    Bar->>Ctrl: addItem(draft, imageBytes)
    Ctrl->>Repo: addItem(groupId, draft, order)
    Repo->>DB: collection(groups/{gid}/items).add(data)
    DB-->>Repo: DocumentReference
    Repo-->>Ctrl: Future<String> (new id)
    Ctrl-->>Bar: AddItemOutcome
    Bar->>User: SnackBar「追加しました！」
    Note over DB,Bar: 別経路: snapshots() が変更を push
    DB-->>Repo: snapshot
    Repo-->>Bar: itemsProvider 経由で再描画
```

`itemsProvider`（`lib/presentation/providers/item_providers.dart`）が `ItemRepository.watchItems` の `Stream` を購読しているため、追加直後のリスト反映は Firestore の `snapshots()` コールバックを介して自動で行われる。UI（`DashboardAddBar` / `DashboardScreen`）の追加ハンドラ自身はアイテム一覧の state を書き換えない。

> **アイテム書き込みの集約（#69）**: アイテムの追加・更新・削除・タグ一括変更といった書き込み操作は
> `ItemController`（`lib/presentation/providers/item_controller.dart`、`itemControllerProvider`）に集約し、
> 画面（widget）は Firebase 配線を持たず UI フィードバック（トースト / 確認ダイアログ / 画面遷移）に専念する。
> `ItemController` は失敗時に `AppError` を throw し、画像付き追加の部分失敗は `AddItemOutcome` で表現する
> （`GroupController` / `AuthController` と同方針）。

> 双方向購読の全体像（書き込み経路と購読経路の分離、オフライン挙動）は [データフロー.md (a)](./データフロー.md#a-snapshots-双方向購読) を参照。

> **画像付き追加の 2 段保存（#38）**: 画像を選択した場合、上図の `addItem`（`imageUrl` 空で作成し
> ID を確定）後に Storage へアップロードし、得た https URL で `updateItemDetails` を呼んで `imageUrl`
> を更新する。アップロード失敗時も best-effort としてアイテム本体は残す。`onAdd` のシグネチャは
> `(Item draft, Uint8List? imageBytes)`。
...
| `groups/{groupId}/itemHistory/{eventId}` | グループの `memberIds` に含まれる認証済みユーザー | 不可（`write: if false`。Functions の Admin SDK のみが書き込む。#37） |
| `groups/{groupId}/purchaseHistorySummaries/{nameKey}` | グループの `memberIds` に含まれる認証済みユーザー | 不可（`write: if false`。同上。#37） |
| `groups/{groupId}/suggestions/{weekId}` | グループの `memberIds` に含まれる認証済みユーザー | 不可（`write: if false`。AI 生成を Functions が書き込む。#40 / #41） |
| `groups/{groupId}/favoriteItems/{favoriteId}` | グループの `memberIds` に含まれる認証済みユーザー | 同左（よく買う物テンプレート。#43） |
...
> **`plan` / `planExpiresAt` の書き込み保護（#39）**: `users/{uid}` と `groups/{groupId}` の write は
> `planFieldsUnchanged()` を AND 条件に課し、クライアントからのプラン変更を禁止する（正本は RevenueCat
> Webhook / Functions）。詳細は [ドメインモデル.md](./ドメインモデル.md) §6 実装メモ。
>
> **商品画像の Storage Rules（#38 / #43）**: 商品画像は Firestore ではなく Firebase Storage
> （`groups/{groupId}/items/{itemId}.jpg`）に保存し、アクセス制御は `storage.rules` で行う
> （グループメンバーのみ読み書き・クロスサービスで Firestore の `memberIds` を参照）。よく買う物
> テンプレート画像（`groups/{groupId}/favoriteItems/{favoriteId}.jpg`、#43）も同一ポリシー。デプロイは
