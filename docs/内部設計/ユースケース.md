...
- **Epic-04 よく買う物リスト**（S04-01〜04 / 機能構想 01）: 定番品テンプレートからの一括追加 UC。Phase 2 想定。AI 提案 Phase 0 の履歴を候補生成に流用できる。
...
> **実装済みに移行した項目**: アイテムの写真（旧 Epic-05）は #38 で Firebase Storage 化が完了し `UC-I1 / UC-I2` のサブフローとして本実装済み（§3.4・[アーキテクチャ概要 §3](./アーキテクチャ概要.md#3-アイテム追加のシーケンス)）。AI 週次提案（旧「将来拡張」）は #37/#40/#41 で Phase 0〜1 を実装し §3.5 に昇格した。
